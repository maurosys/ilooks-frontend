...
const Products = ({products, commonProducts, totalProducts, totalPage,}: StateProps) => {
  ...
  const {category, subCategory, provider, page, search, p} = router.query;
  ...
  const updateQueryProducts = async () => {
    ...
    try {
      ...
      const response = await api.get(`/product/${category}/${subCategory}`, {params});
  ...
  useEffect(() => {
    ...
    if (category && categories) {
      ...
      const ccate = ccategories.find((c) => c.slugy == category).name;
    ...
    if (subCategory && subcategories) {
      ...
      const ccate = ccategories.find((c) => c.slugy == category).name;
      const csubcate = csubcategories.find((c) => c.slugy == subCategory).name;
      setOrigemProducts(`${ccate} / ${csubcate}`);
    }
    if (provider) {
      setOrigemProducts('Produtos');
    }
    if (!category && !subCategory && !provider) {
      setOrigemProducts('Todos os produtos');
    }
  }, [currentPage, productForPage, category, subCategory]);
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  const {category, subCategory, provider} = ctx.query;
  ...
  try {
    const response = await apiClient.get(`/product/${category}/${subCategory}`, {
      params: {
        limit: LimitForPage,
      },
    });
