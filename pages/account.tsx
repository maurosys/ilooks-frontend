	...
	const loadData = async () => {
		...
		if (user_session) {
			getUserFromId(user_session.userId, user_session.token)
				.then((user: any) => {
					user.data.birthday = new Date(user.data.birthDate).toLocaleString('pt-BR', dateOptions);
					user.password = '';
					...
					<div className="container-order">
						...
						<div className="container-order-content">
							<Form>
								...
								/>
								{errorName && (
									<p style={errorStyle}>{errorName}</p>
								)}
								...
								<div className="row">
									<div className="col-lg-6 col-md-12">
										...
										/>
										{errorDocument && (
											<p style={errorStyle}>{errorDocument}</p>
										)}
									</div>
									<div className="col-lg-6 col-md-12">
										<InputText
										...
										/>
										{errorBithday && (
											<p style={errorStyle}>{errorBithday}</p>
										)}
									</div>
								</div>
								
								<ButtonPrimary type="button" loading={loading} onClick={submitData}>
